...
class Oracleutils(object):
    ...
    @staticmethod
    def executeQuery(log, db_ip, db_user, db_pass, db_port, db_sid, db_query, db_output_file):
        dbout = open(db_output_file, 'w')
        try :
            log.setLog('INFO', 'Trying to Connect DB[%s]...' % (db_ip))
            con = cx_Oracle.connect(db_user, db_pass, '%s:%s/%s' % (db_ip, db_port, db_sid))
            ...
            Oracleutils.abort(log)

        try:
            log.setLog('INFO', 'Executing DB Query...')
            cur = con.cursor()
